...
import { getLoginSchema } from "../../utils/loginSchema";
import Link from "next/link";
...
export default function Login() {
    ...
    const formik = useFormik({
        ...
        validationSchema: getLoginSchema(locale),
        onSubmit: async (values) => {
            try {
                setError("");
                setIsLoading(true);
                ...
                if (res.status && res.data?.token) {
                    ...
                    setError(res.message || t('errors.loginFailed', locale));
                }
            } catch (error: any) {
                console.error("Login error:", error);
                setError(
                ...
                <div className="bg-white rounded-2xl shadow-md overflow-hidden">
                    <div className="bg-white p-6 text-center text-black">
                        <h1 className="text-2xl font-bold text-black">
                            {t('common.welcome', locale)}
                        </h1>
                        <p className="text-black mt-2">
                            {t('common.signInToAccount', locale)}
                        </p>
                    </div>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                ...
                                <button
                                    ...
                                    className="text-blue-600 text-sm hover:text-blue-800"
                                >
                                    {showPassword ? t('common.hide', locale) : t('common.show', locale)}
                                </button>
                            </div>
                            <div className="relative">
                                <input
